...
"""
    ...
    - A lista encadeada é uma estrutura de dados formados por unidades
    de informação chamadas nodos ou nós
    - Cada nodo da lista encadeada tem três partes:  uma, que armazena a informação: a segunda
    que guarda o endereço do nodo anterior; e a terceira, que guarda o endereço para o nodo seguinte da sequência
    - A qualquer momento, temos um conhecimento apenas limitado de onde se encontram
    todos os nodos da lista. Sabemos apenas onde está o primeiro e o último nodo da sequência. Os nodos intermediários precisam ser encontrados partindo-se do primeiro OU do último nodo e percorrendo a sequência.
"""
...
class DoublyLinkedList:
    ...
    """
        Método PRIVADO que encontra o nodo, dada a sua posição
    """
    def __find_node(self, pos):
        ...
        # Se, ao contrário, o nodo estiver na segunda metade da lista, compensa mais fazer um percurso reverso desde o __tail
        else: 
            node = self.__tail
    ...
    def index(self, pos):
        ...
        for pos in range(0, meio + 1):
            ...
            node2 = node2.prev # node1 anda para trás
    ...
    def remove(self, pos):
        ...
        # 2º Caso: remoção no inicio da lista
        if pos == 0:
            # Séra removido o __head da lista
            removed = self.__head
        ...
        # 3º Caso: remoção do último nodo
            ...
            # Se __tail for um nodo valido, não pode ter um sucessor (next)
            if self.__tail is not None: self.__tail.next = None
            # Em caso de remoção do único nodo restante, __tail é None, e __head precisa ser None também.
        ...
        # 4º Caso: remoção de posição intermediária
            ...
            before.next = after
            # O nodo after começa a apontar para trás para o nodo before
            after.prev = before
