...
em_ordem = []
arvore.in_order_traversal(lambda val: em_ordem.append(val))
print('Depois exclusão do 29 (grau 2): ', em_ordem)

arvore.remove(21)

em_ordem = []
arvore.in_order_traversal(lambda val: em_ordem.append(val))
print('Depois exclusão do 21 (grau 0): ', em_ordem)

arvore.remove(1)

em_ordem = []
arvore.in_order_traversal(lambda val: em_ordem.append(val))
print('Depois exclusão do 1 (grau 1): ', em_ordem)

arvore.remove(48)

em_ordem = []
arvore.in_order_traversal(lambda val: em_ordem.append(val))
print('Depois exclusão do 48 (grau 2, raiz): ', em_ordem)

# Observando qual valor se tornou a nova raiz
# Fazendo um percurso pré-ordem e verificando o primeiro valor retornado
pre_ordem = []
arvore.pre_order_traversal(lambda val: pre_ordem.append(val))
print('Novo nodo na raiz tem o valor ', pre_ordem[0])